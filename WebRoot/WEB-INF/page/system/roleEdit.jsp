				...
				<table cellSpacing="0" cellPadding="0" width="90%" align="center"
					...
					<tr>
						<td class="ta_01" colspan=2 align="left" width="100%">全选/全不选<input
							...
							<s:set value="%{''}" scope="request" var="parentCode"></s:set> <s:if
								test="%{#request.xmlList!=null}">
								<s:iterator value="%{#request.xmlList}" var="xml">
									<s:if test="%{#request.parentCode == #xml.parentCode}">
										<s:if test="%{#xml.flag == 1}">
											<input type="checkbox" name="selectoper"
												value="<s:property value='%{#xml.code}'/>" checked>
										</s:if>
										<s:else>
											<input type="checkbox" name="selectoper"
									...
									<s:else>
										<s:set value="%{#xml.parentCode}" scope="request"
											var="parentCode"></s:set>
										<br>
				...
				<table cellspacing="0" align="center" width="100%" cellpadding="1"
					...
					<tr
						...
						<td class="ta_01" align="center" width="20%" height=22
							background="../images/tablehead.jpg">选中</td>
						...
						<s:iterator value="%{#request.userList}" var="user">
							<tr onmouseover="this.style.backgroundColor = 'white'"
								...
								<td style="HEIGHT: 22px" class="ta_01" align="center"
									width="20%"><s:if test="%{#user.flag == 1}">
										<input type="checkbox" name="selectuser"
											value="${user.userID}" checked>
									</s:if> <s:else>
										<input type="checkbox" name="selectuser"
											value="${user.userID}">
									</s:else></td>
								<td style="HEIGHT: 22px" class="ta_01" align="center"
									width="40%"><s:property value="%{#user.loginName}" /></td>
								<td style="HEIGHT: 22px" class="ta_01" align="center"
									width="40%"><s:property value="%{#user.userName}" /></td>
							</tr>
						</s:iterator>
					</s:if>
				</table>
